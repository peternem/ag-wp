	...
	<div id="knobOptions" class="col-md-12 type">
		<div class="row">
			...
			if(get_field('knob_options_label')) { ?>
				...
				<?php
				echo '<h2>' . get_field('knob_options_label') . '</h2>';
				?>
				</header>
			<?php } ?>
			</div>
		...
		<div class="row">
			<?php 
			while( have_rows('knob_options') ): the_row(); 
			$image = get_sub_field('image');
			$label = get_sub_field('label');
			$description = get_sub_field('description');
			?>
	...
	<div id="comboOptions" class="col-md-12 type"> 
		<div class="row">
			...
			if(get_field('combo_options_label')) { ?>
				<header class="entry-header page-header">
				<?php
				echo '<h2>' . get_field('combo_options_label') . '</h2>';
				?>
				</header>
			<?php } ?>
			</div>
		...
		<div class="row">
			<?php 
			while( have_rows('combo_options') ): the_row(); 
			$image = get_sub_field('image');
			$label = get_sub_field('label');
			$description = get_sub_field('description');
			?>
